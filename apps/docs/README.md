...
Docs are served from the root (`/`, `/quickstart`, …) so the site can be
hosted at `docs.mikoto.io` directly with no URL prefix.
...
| `app/(docs)/[[...slug]]/page.tsx` | Renders a single MDX page (catches `/`).       |
